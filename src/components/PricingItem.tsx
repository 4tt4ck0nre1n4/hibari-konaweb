...
export function PricingItemButton({ item, isSelected, onToggle, quantity, selectedFunctions }: PricingItemButtonProps) {
  // アニメーション項目かどうか判定
  ...
  const renderIcon = () => {
    ...
    const cssColor =
      isMonochrome && item.iconColorOverride !== undefined ? CSS_COLOR_MAP[item.iconColorOverride] : undefined;
    const colorReplace =
      !isMonochrome && item.iconColorOverride !== undefined ? COLOR_REPLACE_MAP[item.iconColorOverride] : undefined;

    return (
      <div className={`pricing-item__icon ${item.iconClass ?? ""}`}>
        <IconifyInline icon={iconValue} width="40" height="40" colorReplace={colorReplace} cssColor={cssColor} />
      </div>
    );
  };
      ...
      <div className="pricing-item__content">
        ...
        <div className="pricing-item__text">
          ...
          {item.id !== "other-functions" && isSelected && quantity !== undefined && quantity > 0 && (
            <span className="pricing-item__line">{getQuantityLabel()}</span>
          )}
        </div>
      </div>
      {isSelected && <div className="pricing-item__badge">選択中</div>}
    </button>
  );
}

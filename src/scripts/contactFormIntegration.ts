...
export function attachPDFToContactForm(): void {
  ...
  fetch(pdfData.estimatePDF)
    ...
    .then((blob) => {
      ...
      // ファイル入力フィールドを取得
      const fileInput = document.querySelector<HTMLInputElement>('input[name="estimate-pdf"]');
      ...
      if (fileInput) {
        ...
        // 見積番号を自動入力
        const estimateInput = document.querySelector<HTMLInputElement>('input[name="estimate-number"]');
        if (estimateInput) {
